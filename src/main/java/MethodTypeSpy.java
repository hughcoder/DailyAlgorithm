...
public class MethodTypeSpy {
    ...
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        for (Method declaredMethod : declaredMethods) {
            System.out.format(fmt, "Method name", declaredMethod.getName());  //获得单独的方法名
            //获得完整的方法信息（包括修饰符、返回值、路径、名称、参数、抛出值）
            System.out.format(fmt, "toGenericString", declaredMethod.toGenericString());

            int modifiers = declaredMethod.getModifiers();      //获得修饰符
            System.out.format(fmt, "Modifiers", Modifier.toString(modifiers));
